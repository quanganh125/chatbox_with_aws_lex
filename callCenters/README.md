...
2. If you have never created a Call Center, click &quot;Add an instance&quot;
3. At very first Set identity step. Select "Identity management" then enter a custom Access URL (This is a unique name so you will might need to be creative) then click 'Next'
4. Next we will add administrator. This step is optional so you can choose "No administrator" or specify an administrator if you want. Then click 'Next'
5. For this lab you will not need to make outgoing calls, uncheck this option and click 'Next'
6. Data storage step: The Connect application will automatically create you a S3 bucket and encrypt the data. Click 'Next'
7. Finally review the information again click 'Create Instance'. This may take a minute or two to setup.

![Create your Connect Instance](../images/picture_16.png)

8. Once it is setup you will be presented with a screen which allows you to 'Get Started'
9. From home page. click 'Begin' in 'Explore your channels of communication' section
10. At this point your Call Center is setup but you don't have any numbers so you need to 'Claim Phone number'. This is the phone number customers will call to reach your business - For this lab chose a phone number that you can actually call. Just select one from the list the number where you currently live in and select DID (Direct Inward Dialing).
11. Your browser will pop up a warning to ask you show notifications - this is to allow the Connect Application to notify you when a call is coming in. Click allow on the pop up.

![Create your Connect Instance](images/Picture01b.png)

12. You now can call the number and go through the default first call example that is part of Amazon Connect by ringing the number that is allocated to you or click 'Skip for now'.
